// Mobile menu (same as services.js)
...
// FAQ Accordion (same as services.js)
...
/* ---------------------------
  Creative Services Data (FULL — same as services.js)
--------------------------- */
const creative = [
  {
    key: "logo",
...
function packageCard(pkg, isPopular){
  ...
  return `
  <article class="price-card hover-glow glass ${isPopular ? "popular" : ""}">
...
// Needs list (same as services.js)
const itNeeds = [
...
// Partnership/Sponsorship list (kept since you asked for it earlier)
const partnerNeeds = [
  "Partnership proposal",
  "Sponsorship placement",
  "Joint campaign / collaboration",
  "Event partnership"
];
...
function fillNeedOptions(category){
  ...
  if (category === "IT Services"){
    options = itNeeds;
    if (companySizeEl) companySizeEl.disabled = false;
  ...
  } else if (category === "Partnership/Sponsorship"){
    options = partnerNeeds;
    if (companySizeEl) {
      companySizeEl.disabled = true;
      companySizeEl.value = "";
    }
